...
def main():
    ...
    switching_point = np.argmin(np.abs(diffusion_2.alphas_cumprod - (1 - float(args.first_step_beta))))
    # model.switching_point = 63#switching_point
    model_1.unet_1.load_state_dict(
    ...
    with th.no_grad():
        while len(all_images) * args.batch_size < args.num_samples:
            sample = diffusion_2.p_sample_loop(
                model_2,
                (args.batch_size, args.in_channels, args.image_size, args.image_size),
                clip_denoised=args.clip_denoised,
                model_kwargs=model_kwargs,
                limit_indices=switching_point
            )

            sample = diffusion_1.p_sample(
                model = model_1,
                x = sample,
                t = th.zeros(args.batch_size).long().to(dist_util.dev()),
                clip_denoised=args.clip_denoised,
                model_kwargs=model_kwargs
            )["sample"]
